...
use crate::model::urban_network::StreetNode;
...
impl VisualizationState<UrbanNetworkState> for VisState {
    fn on_init(
        &self,
        commands: &mut Commands,
        _sprite_render_factory: &mut AssetHandleFactoryResource,
        state: &mut UrbanNetworkState,
        _schedule: &mut Schedule,
        _sim: &mut SimulationDescriptor,
    ) {
        UrbanNetworkState::init_network_graphics(state, commands)
    }

    fn get_agent_render(
        &self,
        agent: &Box<dyn Agent>,
        _state: &UrbanNetworkState,
    ) -> Option<Box<dyn AgentRender>> {
        Some(Box::new(NetNodeVis {
            id: agent.downcast_ref::<PedAgent>().unwrap().id,
        }))
    }
    ...
    fn get_agent(
        ...
        let state = state.as_any().downcast_ref::<UrbanNetworkState>().unwrap();
        match state.network.0.get_object(agent_render.get_id()) {
